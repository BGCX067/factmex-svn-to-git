...
label0:
        ...
        do
        {
            if(k >= abyte0.length)
                break;
            int l = i >= abyte0.length - k ? abyte0.length - k : i;
 ...
 /**
    public boolean Verificar(BufferedInputStream certificado, String cadena_original, byte sello[])
    {
        ...
        try {
            if (verificaFirma(x509certificate.getPublicKey(), cadena_original.getBytes(), sello)) {
                //break MISSING_BLOCK_LABEL_138;}
                 System.out.println("El Sellado es valido V2...");
                 flag = true;
            }
            System.out.println("El Sellado no es valido V2...");
        } catch (SgiLibException ex) {
            Logger.getLogger(GeneraSello.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public boolean Verificarx509(X509Certificate certificate, String cadena_original, byte sello[])
    {
        boolean flag = false;
        System.out.println("Comprobando validez de sello de manera directa...");
        System.out.println((new StringBuilder("V2 ")).append(new String(certificate.getSerialNumber().toByteArray())).toString());
        System.out.println((new StringBuilder("V2 ")).append(certificate.getNotBefore()).toString());
        System.out.println((new StringBuilder("V2 ")).append(certificate.getNotAfter()).toString());
        try {
            if (verificaFirma(certificate.getPublicKey(), cadena_original.getBytes(), sello)) {
  ...
  public boolean Verificar(BufferedInputStream certificado, String cadena_original, byte[] sello)
  {
    ...
    {
      x509certificate = cargaCert(certificado);
      System.out.println("V2 " + new String(x509certificate.getSerialNumber().toByteArray()));
      System.out.println("V2 " + x509certificate.getNotBefore());
      System.out.println("V2 " + x509certificate.getNotAfter());
      if (verificaFirma(x509certificate.getPublicKey(), cadena_original.getBytes(), sello)) {
        System.out.println("El Sellado es valido V2...");
        return true;
      }
  ...
  public boolean Verificarx509(X509Certificate certificate, String cadena_original, byte[] sello)
  {
    ...
    try
    {
      System.out.println("V2 " + new String(certificate.getSerialNumber().toByteArray()));
      System.out.println("V2 " + certificate.getNotBefore());
      System.out.println("V2 " + certificate.getNotAfter());
      if (verificaFirma(certificate.getPublicKey(), cadena_original.getBytes(), sello)) {
        System.out.println("El Sellado es valido V2...");
        return true;
      }
